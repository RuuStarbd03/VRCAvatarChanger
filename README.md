...
- **サムネイルのデコードは UI スレッドの外**で行う(1 枚あたり数ミリ秒。数百体だと操作が引っかかるため)。
  展開するのは表示に必要な幅だけ(リスト表示 128px / ボックス表示 320px = 300 体でおよそ 14MB / 87MB)。
  メモリ上に持つのは 400 枚まで。あふれたぶんはディスクキャッシュから読み直す
- **サムネイルは「画面に出ているものから」読む**。一覧を開いた瞬間に数百件を取りに行かず、
  表示中のぶんを 4 本並列で先に読み、残りは 1 件ずつ裏で埋める(すぐ閉じたなら、残りは取りに行かない)。
  灰色の枠が見えないよう、一覧は画面の前後 1 ページ分(ボックス表示は前後 2 行)まで先に作って読み始める。
  表示形式を切り替えたときは、今出ている画像を残したまま大きいものに差し替える
...
- **トレイ常駐中 (ウィンドウを閉じている間) はサムネイルの裏埋めを止める**。開いた時点で再開する。
  クイック着替えからの要求だけは常に通す
- **メモリ上の画像は足すたびに整理する**。上限 (400 枚) を超えたら、一覧に無いものと、
  今の表示形式では使わない幅のものを落とす。リストとボックスを行き来しても増え続けない
- **表示形式を切り替えたときに大きい版へ差し替えるのは、画面に出たものだけ**。
  見てもいない数百枚を作り直さない (残りはスクロールして見えたときに差し替わる)
