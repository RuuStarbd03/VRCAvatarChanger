...
- 配色の切り替え(システム / ライト / ダーク)。設定から選ぶとその場で切り替わる
...
- 名前・作者名・ID で絞り込み
- **アバターの詳細**: 右クリック →「詳細を見る...」で、大きい画像・説明文・対応プラットフォーム (PC / Android / iOS)・
  パフォーマンスランク・ダウンロードサイズ・作成日 / 更新日・タグ / グループ / ホットキーをまとめて表示。そこから着替え・VRChat のページを開く・ID コピーができる
- **使えなくなったアバターの検出** (パブリック): 「再読み込み」のたびに登録したアバターを確かめ、削除・非公開化で着替えられなくなったものに
  「使えません」の印 (赤い札 + サムネイルに ⊘) を付け、どの並び順でも末尾に寄せる。フィルタに「使えなくなったもの」が出る。
  VRChat 側の一時的な不調と取り違えないよう、時間を空けて 2 回続けて「見つからない / 非公開」だったものだけを確定し、自動では外さない
  (右クリック →「使えなくなったものをまとめて外す...」、または詳細画面から外す。着替えようとしたときも確認が出る)
- 一覧から選択(またはダブルクリック)して着替え
- 100 体を超える一覧は分割して取得するが、取得中に並びが変わっても取りこぼさないようにしている
  (ページを重ねて取り、ID で重複を除き、最後に先頭をもう一度確認する)
- **キーボード操作**: Enter で着替え / グループを開く、Ctrl+I で詳細、Ctrl+F で検索、Ctrl+1〜3 と Ctrl+Tab でタブ、
  Ctrl+G でグループ化、Ctrl+Shift+L で表示形式、Ctrl+C で ID コピー、Delete でパブリックから外す、Esc / Backspace で戻る、F5 で再読み込み、Ctrl+, で設定
- **直前のアバターに戻す**: ホットキーで 2 着を行き来できる
...
- **ログの記録**(設定): `%AppData%\VRCAvatarChanger\app.log` に動作を時刻つきで残す。エラーは常に、警告・情報は設定がオンのときだけ。
  2MB で `app.log.1` に退避(最大 2 世代)。設定の「開く」でそのまま開ける。パスワード・トークン・クッキーは書かない。
  設定やグループなどの JSON が壊れていた場合は黙って既定値に戻さず、`*.broken-日時` として退避したうえで画面とログに伝える
...
- **取れなかったサムネイルは間を置いて取り直す**(3 秒・6 秒・9 秒の 3 回まで)。
  取得の要求はタイルが画面に出た瞬間にしか出ないため、拾わないと「画面に出たままのタイルが空のまま」になる
  (一斉に読み込んでレート制限に当たったときに起きる)
...
- **配色**: 設定の「配色」で システム / ライト / ダーク を選べます。既定の「システム」は Windows の
  「アプリのモード」に追従し、Windows 側で切り替わった時点で(時間帯による自動切替も含めて)その場で変わります。
  選び直したときも再起動は不要です。タイトルバーも同色。検証用に環境変数 `VRCAC_THEME=light|dark` で強制できます(設定より優先)。
