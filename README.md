...
VRChat の OSC では外部からアバターを**切り替えることはできません**(パラメータ操作と、切り替わった通知の受信のみ)。
そのため着替えは Web API、現在のアバター表示は OSC(既定 `127.0.0.1:9001` を待ち受け)という組み合わせになっています。
OSC 受信には VRChat 側で OSC を有効にしてください(アクションメニュー → Options → OSC → Enabled)。
...
- **10 体ごとの色分け**: メイン画面で `Ctrl+Shift+C`。表示順に 10 体ごとにアバターの背景色が変わる(6 色ループ)。
  グループタイルは中のアバター数ぶんカウントする(グループ内で個別に除外した分は数えない)。
  有効中は右クリックに「カウントから除外」が出て、除外したものは数に入らず色も付かない。
...
| `src\OscListener.cs` | OSC 受信(`/avatar/change` の検知、依存ライブラリなし) |
